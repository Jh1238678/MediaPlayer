...
{
    //m_playerState = VideoPlayer::PlayerState::Stop;
    m_videoState.quit = 1;
    if( isWait ) //阻塞标志
    {
...
VideoPlayer::PlayerState VideoPlayer::playerState() const
{
...
int video_thread(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            if( is->quit) break;
            if( is ->audioq->size == 0 ) break;//防止没声音一直无法退出同步
...
qint64 VideoPlayer::getTotalTime()
{
//    if( m_videoState.pFormatCtx )
//        return m_videoState.pFormatCtx->duration;
//    return -1;
    if(m_videoState.pFormatCtx->duration != AV_NOPTS_VALUE)
    {
        qint64 duration = m_videoState.pFormatCtx->duration + 5000;
        return duration;
    }
    return -1;
}
...
void VideoPlayer::run()
{
    ...
    if( avformat_open_input(&pFormatCtx, file_path, NULL, NULL) != 0 )
    {
        ...
        avformat_free_context(pFormatCtx);//后改
        return;
    }
    //3.1 获取视频文件信息
    if (avformat_find_stream_info(pFormatCtx, NULL) < 0)
    {
        ...
        avformat_close_input(&pFormatCtx);//后改
        return;
    }
    //4.查找文件中的视频流
    ...
    if (videoStream != -1) {
        ...
        //而且我们并不知道他实际用的是什么编码器。
        //这就是为什么一开始我们使用 FFMPEG 来操作，因为很多东西我们可以不关心。
        //6.申请解码需要的结构体
    ...
    std::cout<<"Q_EMIT SIG_TotalTime( getTotalTime() );"<<std::endl;
